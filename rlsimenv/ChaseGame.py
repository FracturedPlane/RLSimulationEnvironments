...
class ChaseGame(Environment):
    ...
    def reachedTarget(self, agent):
        # Might be a little touchy because floats are used
        a=(self._agent[agent] - self._target)
        d = np.sqrt((a*a).sum(axis=0))
        return d <= 0.3
    ...
    def endOfEpoch(self):
        num_ = 0
        for a in range(self._numberOfAgents):
            num_ += self.reachedTarget(a)
        
        if ( num_ > (self._numberOfAgents/2.0)):
            return True
        else:
            return False
