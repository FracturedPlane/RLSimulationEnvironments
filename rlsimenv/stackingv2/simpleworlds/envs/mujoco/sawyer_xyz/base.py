...
#         prev_state.qpos[:len(joint_state.qpos)] = joint_state.qpos
#         prev_state.qvel[:len(joint_state.qvel)] = joint_state.qvel
...
class SawyerXYZEnv(SawyerMocapBase, metaclass=abc.ABCMeta):
    ...
    def set_xyz_position(self, pos):
        x,y,z = pos
        new_mocap_pos = np.array([[x, y, z]])
        new_mocap_pos[0, :] = np.clip(
            new_mocap_pos[0, :],
            self.mocap_low,
            self.mocap_high,
